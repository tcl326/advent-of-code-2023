...
def parse_card(line: str) -> Tuple[int, Set[int], Set[int]]:
    ...
    wining_numbers_str, numbers_str = num_str.split("|")
    wining_numbers = set([int(w) for w in wining_numbers_str.split()])
    numbers = set([int(w) for w in numbers_str.split()])

    return card_id, wining_numbers, numbers

def part1(cards: List[Tuple[Set[int], Set[int]]]):
    res = 0
...
if __name__ == "__main__":
    file_name = "d4-input.text"

    cards = []
    with open(file_name, "r") as f:
        lines = f.readlines()
        for line in lines:
            _, winning, numbers = parse_card(line)
